...
$consultaLivros = pg_query($conn, "SELECT nome, ano, autor, qtd FROM livros ORDER BY nome");
$consultaDiscos = pg_query($conn, "SELECT nome, ano, autor, qtd FROM discos ORDER BY nome");
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
...
<body>
  ...
  $consultaLivros = pg_query_params($conn, $sqlLivros, array('%' . $filtroLivro . '%'));
  ...
  $consultaDiscos = pg_query_params($conn, $sqlDiscos, array('%' . $filtroDisco . '%'));
  ?>
  <header>
    <div class="list">
